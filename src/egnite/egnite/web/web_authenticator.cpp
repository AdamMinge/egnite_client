...
namespace egnite::web
{
  ...
  void
  SimpleJWTAuthenticator::setAccessTokenLifetime(unsigned accessTokenLifetime)
  {
    if (m_accessTokenLifetime == accessTokenLifetime) return;
  ...
  void
  SimpleJWTAuthenticator::setRefreshTokenLifetime(unsigned refreshTokenLifetime)
  {
    if (m_refreshTokenLifetime == refreshTokenLifetime) return;
