...
SimpleJWTAuthenticator::SimpleJWTAuthenticator(QObject *parent)
    : Authenticator(parent),
      m_access_token_lifetime(60 * 60),
      m_refresh_token_lifetime(60 * 60 * 12),
...
unsigned SimpleJWTAuthenticator::getAccessTokenLifetime() const {
  return m_access_token_lifetime;
}

void SimpleJWTAuthenticator::setAccessTokenLifetime(
    unsigned access_token_lifetime) {
  if (m_access_token_lifetime == access_token_lifetime) return;
...
void SimpleJWTAuthenticator::setRefreshTokenLifetime(
    unsigned refresh_token_lifetime) {
  if (m_refresh_token_lifetime == refresh_token_lifetime) return;

  m_refresh_token_lifetime = refresh_token_lifetime;
  Q_EMIT refreshTokenLifetimeChanged(m_refresh_token_lifetime);
}

void SimpleJWTAuthenticator::login(const QString &username,
                                   const QString &password) {
  auto client = getClient();
  Q_ASSERT(client);

  auto reply =
      client->post(client->getUrl(m_routing->gettokenCreate()),
  ...
  reply
      ...
      .onError(std::bind(&SimpleJWTAuthenticator::loginError, this,
                         std::placeholders::_1));
}
...
void SimpleJWTAuthenticator::logout() {
  ...
  reply
      ...
      .onError(std::bind(&SimpleJWTAuthenticator::logoutError, this,
                         std::placeholders::_1));
}
...
void SimpleJWTAuthenticator::renewAccessToken() {
  ...
  auto reply =
      client->post(client->getUrl(m_routing->gettokenRefresh()),
  ...
  reply
      ...
      .onError(
          [](auto error) { qDebug() << "renew token failed = " << error; });
}
...
void SimpleJWTAuthenticator::renewRefreshToken() {
  ...
  // TODO - Research about refreshing refresh token ( maybe stay login and sens
  // login request? )
}
...
void SimpleJWTAuthenticator::updateHeaders() {
  ...
  if (!m_access_token.isEmpty())
    headers->setHeader(Headers::KnownHeaders::Authorization,
                       "Bearer " + m_access_token);
}
