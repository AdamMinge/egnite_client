...
egnite::rest::Reply createReply(QNetworkReply *qreply,
...
Client::Client(QObject *parent)
    : QObject(parent), m_headers(nullptr), m_serializer(nullptr) {}
...
void Client::setSerializer(Serializer *serializer) {
  if (m_serializer == serializer) return;
...
Reply Client::get(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(
      m_network_access_manager.get(m_headers->createRequest(url)), this);
}

Reply Client::post(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "POST"),
                     this);
}
...
Reply Client::post(const QUrl &url, const QByteArray &data) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "POST", data),
                     this);
}
...
Reply Client::put(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "PUT"),
                     this);
}
...
Reply Client::put(const QUrl &url, const QByteArray &data) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "PUT", data),
                     this);
}
...
Reply Client::patch(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "PATCH"),
                     this);
}
...
Reply Client::patch(const QUrl &url, const QByteArray &data) {
  Q_ASSERT(m_headers);
  return createReply(m_network_access_manager.sendCustomRequest(
                         m_headers->createRequest(url), "PATCH", data),
                     this);
}
...
Reply Client::deleteResource(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(
      m_network_access_manager.deleteResource(m_headers->createRequest(url)),
      this);
}
...
Reply Client::head(const QUrl &url) {
  Q_ASSERT(m_headers);
  return createReply(
      m_network_access_manager.head(m_headers->createRequest(url)), this);
}

Reply Client::options(const QUrl &url) {
  Q_ASSERT(m_headers);
  auto reply = createReply(m_network_access_manager.sendCustomRequest(
                               m_headers->createRequest(url), "OPTIONS"),
                           this);
  return reply;
}
