...
JsonArchive::JsonArchive(nlohmann::json& root) : m_root(root) {
...
void JsonArchive::push(const std::string& key) {
  m_stack.push_back(&value()[key]);
}

void JsonArchive::push(int number) {
  auto& target = value();
  if (!target.is_array()) target = nlohmann::json();
...
void JsonOArchive::write_data(nlohmann::json& data,
