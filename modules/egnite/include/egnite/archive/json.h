#ifndef EGNITE_SERIALIZER_JSON_H
#define EGNITE_SERIALIZER_JSON_H
...
#include <vector>
/* ----------------------------------- Local -------------------------------- */
#include "egnite/archive/serializer/wrappers.h"
...
class EGNITE_API JsonOArchive : public detail::common_oarchive<JsonOArchive> {
 ...
 private:
  ...
  template <detail::ConvertableToJson TYPE>
  void save(const TYPE& v) {
  ...
  template <typename TYPE>
  void save_override(const serialization::nvp<TYPE>& nvp) {
    save_start(nvp.name());
    save_override(nvp.const_value());
    save_end(nvp.name());
  }
  template <typename TYPE>
  void load_override(const serialization::array_wrapper<TYPE>& array) {
...
class EGNITE_API JsonIArchive : public detail::common_iarchive<JsonIArchive> {
 ...
 private:
  template <typename TYPE>
  void load(TYPE& v) {}
  template <detail::ConvertableFromJson TYPE>
  void load(TYPE& v) {
    v = read_data();
  }
  ...
  template <typename TYPE>
  void load_override(const serialization::nvp<TYPE>& nvp) {
    load_start(nvp.name());
    if (!m_archive.value().is_null()) load_override(nvp.value());
    load_end(nvp.name());
  }
  template <typename TYPE>
  void load_override(const serialization::array_wrapper<TYPE>& array) {
...
#endif  // EGNITE_SERIALIZER_JSON_H
