...
class EGNITE_API Serializer : public QObject {
 ...
 public:
  ...
  QByteArray serialize(const TYPE& value);
  template <typename TYPE>
  TYPE deserialize(const QByteArray& data);
...
template <typename TYPE>
QByteArray Serializer::serialize(const TYPE& value) {
  switch (m_format) {
    case Format::Json:
...
template <typename TYPE>
TYPE Serializer::deserialize(const QByteArray& data) {
  switch (m_format) {
    case Format::Json:
