...
macro(egnite_add_module target)
    ...
    install(TARGETS ${target} EXPORT egniteConfigExport
...
function(egnite_export_modules)
    ...
    write_basic_package_version_file("${CMAKE_CURRENT_BINARY_DIR}/egnite-config-version.cmake"
    ...
    configure_package_config_file("${current_dir}/egnite-config.cmake.in" "${CMAKE_CURRENT_BINARY_DIR}/egnite-config.cmake"
            INSTALL_DESTINATION "${config_package_location}")
    configure_package_config_file("${current_dir}/egnite-config-dependencies.cmake.in" "${CMAKE_CURRENT_BINARY_DIR}/egnite-config-dependencies.cmake"
            INSTALL_DESTINATION "${config_package_location}")

    install(EXPORT egniteConfigExport
            FILE ${targets_config_filename}
            DESTINATION ${config_package_location})

    install(FILES ${CMAKE_CURRENT_BINARY_DIR}/egnite-config.cmake
            ${CMAKE_CURRENT_BINARY_DIR}/egnite-config-dependencies.cmake
            ${CMAKE_CURRENT_BINARY_DIR}/egnite-config-version.cmake
            DESTINATION ${config_package_location})

    install(DIRECTORY ${EGNITE_SOURCE_DIR}/include
            DESTINATION .
            COMPONENT devel
...
macro(_egnite_add_executable target)
    ...
    if (NOT "${THIS_UNPARSED_ARGUMENTS}" STREQUAL "")
        message(FATAL_ERROR "Extra unparsed arguments when calling _egnite_add_executable: ${THIS_UNPARSED_ARGUMENTS}")
    endif()
...
macro(egnite_add_test target)
    ...
    cmake_parse_arguments(THIS "" "RESOURCES_DIR" "SOURCES;DEPENDS;DEPENDS_PRIVATE" ${ARGN})
    if (NOT "${THIS_UNPARSED_ARGUMENTS}" STREQUAL "")
        message(FATAL_ERROR "Extra unparsed arguments when calling egnite_add_test: ${THIS_UNPARSED_ARGUMENTS}")
    endif()

    _egnite_add_executable(${target}
    ...
    target_include_directories(${target}
            PUBLIC $<BUILD_INTERFACE:${EGNITE_SOURCE_DIR}/tests>)
...
macro(egnite_add_example target)
    ...
    target_include_directories(${target}
            PUBLIC $<BUILD_INTERFACE:${EGNITE_SOURCE_DIR}/examples>)
...
macro(egnite_add_application target)
    ...
    cmake_parse_arguments(THIS "" "RESOURCES_DIR" "SOURCES;DEPENDS;DEPENDS_PRIVATE" ${ARGN})
    if (NOT "${THIS_UNPARSED_ARGUMENTS}" STREQUAL "")
        message(FATAL_ERROR "Extra unparsed arguments when calling egnite_add_application: ${THIS_UNPARSED_ARGUMENTS}")
    endif()
    ...
    target_include_directories(${target}
            PUBLIC $<BUILD_INTERFACE:${EGNITE_SOURCE_DIR}/include>
            PRIVATE $<BUILD_INTERFACE:${EGNITE_SOURCE_DIR}/src>)
...
macro(egnite_add_translations target)
    ...
    if (NOT "${THIS_UNPARSED_ARGUMENTS}" STREQUAL "")
        message(FATAL_ERROR "Extra unparsed arguments when calling egnite_add_translations: ${THIS_UNPARSED_ARGUMENTS}")
    endif()
